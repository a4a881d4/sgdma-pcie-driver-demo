...
//	long iosize_0,iosize_1;
	...
	if(1)
	{
		...
		u32 __iomem * rom_base = ioremap_nocache( pci_resource_start(dev, 1), pci_resource_len(dev, 1) );

		printk("<0> resource_start ==> %lx, ram_base =%lx\n", pci_resource_start(dev,1), rom_base );

		int idx = 0;
		for( idx = 0 ; idx < pci_resource_len( dev, 1 )/4 ; idx ++ )
		{
			printk(	"         rom[0x%x] = 0x%x  0x%x \n" , idx , rom_base[ idx ], ioread32( &rom_base[idx] ) );
			rom_base[idx] = 0x31;	
			printk(	" after - rom[0x%x] = 0x%x  0x%x \n" , idx , rom_base[ idx ], ioread32( &rom_base[idx] ) );
		}
